...
<!-- Action Buttons (Admin Version) -->
@if($data->link_brief || $data->file || ($data->status_upload == 1 && ($data->total_view || $data->total_share || $data->total_likes || $data->total_comments)))
<div class="border-top pt-3">
    <div class="d-flex gap-2 flex-wrap">
        @if($data->link_brief)
            <a href="{{ $data->link_brief }}" target="_blank" class="btn btn-primary btn-sm">
                <i class="fa fa-external-link-alt me-1"></i> Open Brief
            </a>
        @endif
        @if($data->file)
            <a href="{{ $data->file }}" target="_blank" class="btn btn-success btn-sm">
                <i class="fa fa-download me-1"></i> Download
            </a>
        @endif
        @if($data->status_upload == 1)
            <button type="button" class="btn btn-info btn-sm" onclick="openEngagementModal({{$data->id}}, '{{$data->nama}}')">
                <i class="fa fa-chart-line me-1"></i> Update Engagement
            </button>
        @endif
    </div>
</div>
