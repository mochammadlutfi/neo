    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-content p-4">
                <form method="POST" action="{{ route('admin.paket.update', $data->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-4">
                        ...
                        <div class="col-4">
                            <x-input-field type="text" id="kode" name="kode" label="Kode" :required="true" value="{{ $data->kode }}"/>
                        </div>
                        <div class="col-4">
                            <x-input-field type="text" id="harga" name="harga" label="Harga / Bulan"  value="{{ $data->harga }}" :required="true"/>
                        </div>
                    </div>
                    <div class="form-floating mb-4">
                        <textarea class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" style="height: 141px"
                            id="field-deskripsi" name="deskripsi" placeholder="Deskripsi">{{ old('deskripsi', $data->deskripsi) }}</textarea>
                            <label class="form-label" for="field-deskripsi">Deskripsi</label>
                        <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
                    </div>
                    <x-input-field type="text" id="fitur" inputClass="d-flex fs-xs p-1" name="fitur" label="Fitur" value="{{ $data->fitur }}" :required="true"/>
